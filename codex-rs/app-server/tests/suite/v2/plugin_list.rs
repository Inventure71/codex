  ...
  "plugins": [
    ...
    assert!(
        response
            .marketplaces
            .iter()
            .any(|marketplace| marketplace.path
                == codex_home.path().join(".agents/plugins/marketplace.json"))
    );
    Ok(())
}
...
"#,
    ...
    let marketplace = response
        ...
        .into_iter()
        .find(|marketplace| {
            marketplace.path == codex_home.path().join(".agents/plugins/marketplace.json")
        })
        .expect("expected home marketplace entry");

    assert_eq!(marketplace.plugins.len(), 1);
    assert_eq!(marketplace.plugins[0].name, "shared-plugin");
    assert_eq!(marketplace.plugins[0].enabled, true);
    Ok(())
}
